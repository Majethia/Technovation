...
function Registration() {
  ...
  const [escapeRoomPrice, setEscapeRoomPrice] = useState(0);
  const [mainEvent, setMainEvent] = useState(250);
  ...
  const [mainEventchoice, setMainEventchoice] = useState('');
  ...
  const handleSubmit = (event) => {
    ...
    formData.append('mainEvent', mainEventchoice);
    formData.append('escapeRoom', escapeRoomPrice);

    setLoading(true);
    axios.post('http://dsaii.co.in:8000/api/register/', formData, { headers: { 'content-type': 'multipart/form-data' }})
      .then(response => {
            ...
            <form onSubmit={handleSubmit} className="flex justify-center items-center flex-col sm:inline-block">
              <div className="mb-5 mt-10">
                ...
                <div className=' bg-slate-400/20 px-5 py-4 rounded-md'>
                  ...
                  <input type="radio" id="technova" name="technova" value="technova" className=' mx-3' onChange={() => { setMainEventchoice('Tech-A-Thon') }} />
                  <label htmlFor="nerf_arena" className=' font-montserrat font-bold text-base text-slate-100 uppercase'>TechNova</label>
                  <input type="radio" id="technova" name="technova" value="technova" className=' mx-3' onChange={() => { setMainEventchoice('TechNova') }} />
                </div>
              ...
              <div className="mb-5">
                <label htmlFor="name" className="block mb-2 text-sm font-medium text-slate-100 font-Android_Assassin uppercase">college name</label>
                <input autoComplete='off' type="text" className=" bg-slate-800/30 border border-slate-400/20 p-3 rounded-lg focus:outline-none focus:ring focus:ring-slate-400/30 w-80 md:w-full focus:bg-slate-800/30 text-slate-100 text-base" onChange={e => setCollegeName(e.target.value)} placeholder='College name'/>
              </div>

              <div className="mb-5 mt-5">
                <label htmlFor="name" className="block mb-2 text-sm font-medium text-slate-100 font-Android_Assassin uppercase w-80">Do you want to participate in Mini Events</label>
                <div className=' bg-slate-400/20 px-5 py-4 rounded-md'>
                  <label htmlFor="escape_room" className=' font-montserrat font-bold text-base text-slate-100 mr-3'>Escape Room :-</label>
                  <label htmlFor="escape_room" className=' font-montserrat font-bold text-base text-slate-100'>Yes</label>
                  <input type="radio" name="escape_room" id="escape_room" value="escape_room" className=' mx-3' onChange={() => {setEscapeRoomPrice(200)}}/>
                  <label htmlFor="escape_room" className=' font-montserrat font-bold text-base text-slate-100'>No</label>
                  <input type="radio" name="escape_room" id="escape_room" value="escape_room" className=' mx-3' onChange={() => { setEscapeRoomPrice(0) }} />
                </div>
              </div>
              ...
              <div className=' flex justify-center items-center flex-col mt-10 mb-7'>
                ...
                <p className=' font-Android_Assassin text-base text-slate-200 mt-2'>Please pay {mainEvent + escapeRoomPrice}</p>
              </div>
